...
# Create json data out of .png enc_64x64_images and text files
...
from models.char_embedding.char_cnn_rnn import CharCnnRnn, prepare_text

embeddings=[]

def main():
    parser = argparse.ArgumentParser(description='Fix enc_64x64_images name and create json texts.')
    ...
    output_folder = output_folder + '/prep_data'
    # Create output dir
    ...
    for img in glob.glob(input_folder + '/*.jpg'):
        ...
        # Read and save text
        f_txt=open(input_folder + '/' + text, 'r',encoding='utf-8')
        data = f_txt.read()
        ...
        img_des = output_folder + '/original_images/' + img_id + '.jpg'
        json_img['id'] = img_id
        # json_img['img_path'] = '/original_images/' + img_id + '.jpg'
        json_img['text'] = data
        # Copy original image
        ...
        # son_img['encod_64x64_path'] = '/enc_64x64_images/' + name_enc_img_64
    ...
    pickle.dump(embeddings, open(output_folder + "/char-cnn-rnn-embeddings-test.pt", 'wb'), protocol=0)
...
def embedding_text(text, img_id, output_folder, embedding, device):
    ...
    embedded_txt_array=embedded_txt.cpu().numpy()
    embeddings.append(embedded_txt_array)
...
def scale_img(img, output_folder):
    ...
    if len(sized_img.shape) == 2 or sized_img.shape[2] == 1:
        # sized_img = cv2.cvtColor(sized_img, cv2.COLOR_BGR2GRAY)
        sized_img = np.stack((sized_img,) * 3, axis=-1)
