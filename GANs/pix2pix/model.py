...
def load_data(filename_path, class_id_path, dataset_path, embeddings_path, size):
	...
	for i in progress(range(len(filenames))):
		...
		e = embeddings[i, :, :]
		# if containing 10 rows
		# embed_index = np.random.randint(0, e.shape[0] - 1)
		# embed = e[embed_index, :]
		# else if containing 1row
		embed = e[0, :]
...
class StackGanStage1(object):
	...
	def visualize_stage1(self):
		...
		"""
		tb = TensorBoard(log_dir="logs/stage1")
	...
	def train_stage1(self):
		...
		file_g_loss=open('metrics/stage1_g_loss.csv','w',encoding='utf-8')
		file_d_loss = open('metrics/stage1_d_loss.csv', 'w', encoding='utf-8')
		for epoch in range(self.epochs):
			...
			# save check points
			self.checkpoint1.save(f'checkpoints/stage1/stage1-{epoch}.ckpt')
			file_d_loss.write(f'd_loss\t{epoch}\t{np.mean(dis_loss)}\n')
			g0=[row[0] for row in gen_loss]
			g1 = [row[1] for row in gen_loss]
			g2 = [row[2] for row in gen_loss]
			file_g_loss.write(f'g_loss\t{epoch}\t{np.mean(g0)}\t{np.mean(g1)}\t{np.mean(g2)}\n')
			file_g_loss.flush()
			file_d_loss.flush()
		file_g_loss.close()
		file_d_loss.close()
...
class StackGanStage2(object):
	...
	def visualize_stage2(self):
		"""Running Tensorboard visualizations.
		"""
		tb = TensorBoard(log_dir="logs/stage2/")
	...
	def train_stage2(self):
		...
		real = np.ones((self.batch_size, 1), dtype='float') * 0.9
		fake = np.zeros((self.batch_size, 1), dtype='float') * 0.1
		file_g_loss = open('metrics/stage2_g_loss.csv', 'w', encoding='utf-8')
		file_d_loss = open('metrics/stage2_d_loss.csv', 'w', encoding='utf-8')
		for epoch in range(self.epochs):
			print(f'Epoch: {epoch}')
			...
			# save check points
			self.checkpoint2.save(f'checkpoints/stage2/stage2-{epoch}.ckpt')
			# save loss for analysis
			file_d_loss.write(f'd_loss\t{epoch}\t{np.mean(disc_loss)}\n')
			g0 = [row[0] for row in gen_loss]
			g1 = [row[1] for row in gen_loss]
			g2 = [row[2] for row in gen_loss]
			file_g_loss.write(f'g_loss\t{epoch}\t{np.mean(g0)}\t{np.mean(g1)}\t{np.mean(g2)}\n')
...
if __name__ == '__main__':
	...
	embeddings_path_train = train_dir + "/char-CNN-RNN-embeddings-train.pt"
	embeddings_path_test = test_dir + "/char-CNN-RNN-embeddings-test.pt"
	filename_path_train = train_dir + "/filenames.pickle"
	filename_path_test = test_dir + "/filenames.pickle"
	class_id_path_train = train_dir + "/class_info.pickle"
	class_id_path_test = test_dir + "/class_info.pickle"
	dataset_path = "med_images_single_cat"

	# stage1 = StackGanStage1()
	# stage1.visualize_stage1()
	# stage1.train_stage1()
